...
class SpritesEnv(gym.Env):
    ...
    def step(self, action):
        vel = np.array(action) * self.max_speed #Ziyi max_speed=0.05
        state = self._state.copy()
        state[0,2:] = vel
        pos_state, self._state = self.forward(state)

        im = self._render(np.expand_dims(pos_state, 0), self.shapes).squeeze(0) #im (64, 64)
...
class SpritesStateEnv(SpritesEnv):
    ...
    def step(self, action):
        ...
        return self._state[:, :self._n_dim].copy().flatten(), reward, done, im
        #return {'o':self._state[:, :self._n_dim].copy().flatten(), 'r':reward, 'd':done, 'info':im}
...
if __name__  == '__main__':
    ...
    obs = env.reset()
    #cv2.imwrite("test_rl.png", 255 * np.expand_dims(obs, -1)) #Ziyi
    obs, reward, done, info = env.step([0, 0])
    #cv2.imwrite("test_rl_1.png", 255 * np.expand_dims(obs, -1)) #Ziyi
