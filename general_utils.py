...
def get_padding(seq, replace_dim, size, val=0.0):
    """Returns padding tensor of same shape as seq, but with the target dimension replaced to 'size'.
       All values in returned array are set to 'val'."""#Ziyi: padding between frames (the white area)
    seq_shape = seq.shape
...
def stack_with_separator(tensors, dim, sep_width=2, sep_val=0.0):
    ...
    tensors = [(t + 1.0) / 2.0 if t.min() < 0.0 else t for t in tensors] #Ziyi: tensor range (0,1), change from (t+1)/2
...
def make_image_seq_strip(imgs, n_logged_samples=5, sep_val=0.0):
    ...
    plot_imgs = stack_with_separator(imgs, dim=3, sep_val=sep_val)[:n_logged_samples] # plot_imgs (1, 30, 3, 64, 64)
    return stack_with_separator([t[:, 0] for t in np.split(plot_imgs, int(plot_imgs.shape[1] / 1), 1)],
                                dim=3, sep_val=sep_val) #input tensor list: 30 x(1,3,64,64)
