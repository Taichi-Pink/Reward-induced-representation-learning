...
class MovingSpriteDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        data_dict = AttrDict()
        #data_dict.images = traj.images[:, None].repeat(3, axis=1).astype(np.float32) / (255./2) - 1.0 #Ziyi: image ranges from -1 to 1; shape (30,3,64,64)
        data_dict.images = traj.images[:, None].astype(np.float32) / (255. / 2) - 1.0 #Ziyi: input imag_ch=3 -> 1
        data_dict.states = traj.states #shape (30,4,2)
...
####Ziyi wrote on Aug27
class MovingSpriteDataset_OneShape(Dataset):
    """Dataset of one sprite bouncing in frame, contains one reward annotation."""
    def __init__(self, spec):
        self._spec = spec
        self._generator = OneShapeTemplateMovingSpritesGenerator(self._spec)

    def __getitem__(self, item):
        traj = self._generator.gen_trajectory()

        data_dict = AttrDict()
        data_dict.images     = traj.images[:, None].astype(np.float32) / (255. / 2) - 1.0 #shape (T+N, 1, 64, 64)
        data_dict.states     = traj.states #shape (T+N, K ,2)
        data_dict.shape_idxs = traj.shape_idxs
        data_dict.rewards    = traj.rewards
...
class MovingSpritesGenerator:
    ...
    def gen_trajectory(self):#Ziyi: main function
        ...
        output.images = self._render(output.states, shapes) #Ziyi: image value range from 0.0 to 255.0 (float32) shape (T+N, 64, 64)
...
####Ziyi
class OneShapeTemplateMovingSpritesGenerator(TemplateMovingSpritesGenerator):
    """Differentiates between agent, target and distractor shapes."""
    AGENT = 'tri_bottom'

    def _sample_shapes(self):
        """Retrieves shapes for agent."""
...
if __name__ == '__main__':
    ...
    #    shapes_per_traj=4,      # number of shapes per trajectory (Ziyi: 2 for agent and target, and another 2 for distractors)
    #    rewards=[ZeroReward],
    #)
